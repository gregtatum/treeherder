...
/**
 * This file is taken from: https://github.com/firefox-devtools/profiler/blob/master/src/components/shared/VirtualList.js
 *
...
// MIT Licensed Matt DesLauriers
// https://github.com/mattdesl/array-range/blob/master/index.js
function range(start, end) {
  const n0 = typeof start === 'number';
  const n1 = typeof end === 'number';
  ...
  scrollItemIntoView(itemIndex /*:: : number */, offsetX /*:: : CssPixels */) {
    ...
    if (container.scrollTop > itemTop) {
      ...
      container.scrollTop = Math.min(
        itemTop,
        itemBottom - container.clientHeight * 0.5,
      );
    }
      ...
      >
        <div className={`${className}InnerWrapper`}>
          {range(columnCount).map((columnIndex) => (
            <VirtualListInner
              className={`${className}Inner ${className}Inner${columnIndex}`}
